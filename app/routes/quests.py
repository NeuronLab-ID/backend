...
async def execute_quest_code(
    request: Request,
    body: QuestExecuteRequest,
    user_id: int = Depends(get_current_user),
    controller: QuestController = Depends(get_quest_controller),
):
    """Execute code for a quest exercise (requires auth)."""
    return await controller.execute_code(body.problem_id, body.step, body.code)
...
async def generate_test_case_reasoning_route(request: QuestReasoningRequest, user_id: int = Depends(get_current_user)):
    """Generate step-by-step reasoning for a test case."""
    return await generate_test_case_reasoning(request.function_signature, request.test_input, request.expected_output)
...
async def fix_mermaid_code_route(request: FixMermaidRequest, user_id: int = Depends(get_current_user)):
