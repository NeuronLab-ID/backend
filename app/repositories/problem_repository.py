...
class ProblemRepository:
    ...
    def list_problems(
        self, page: int, limit: int, category: str | None = None, search: str | None = None
    ...
    def get_problem_ids_with_quests(self, problem_ids: list[int]) -> set[int]:
        return set(
            q.problem_id for q in self.db.query(Quest.problem_id)
            .filter(Quest.problem_id.in_(problem_ids)).all()
        )
