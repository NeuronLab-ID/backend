...
class ReasoningService:
    ...
    def __init__(self, use_perplexity: bool = False, use_perplexity_reasoning: bool = False):
        """
        ...
        self.reasoning_provider = get_reasoning_provider(use_perplexity=use_perplexity_reasoning)
...
"""
    ...
    async def stream_full_reasoning(
        ...
        if self.search_provider:
            ...
            yield {"type": "search", "data": {"step": 0, "topic": f"Searching: {search_topic[:50]}..."}}
...
async def fix_mermaid_code(code: str, error: str) -> str:
    ...
    try:
        ...
        response = client.chat.completions.create(
            model=AI_MODEL,
            messages=[
                {"role": "system", "content": get_mermaid_fix_system_prompt()},
                {"role": "user", "content": get_mermaid_fix_prompt(code, error)}
            ],
            max_tokens=2000,
            temperature=0.1
        )
        
        fixed_code = response.choices[0].message.content or code
...
async def generate_test_case_reasoning(function_signature: str, test_input: str, expected_output: str) -> dict:
    ...
    try:
        ...
        response = client.chat.completions.create(
            model=AI_MODEL,
            messages=[
                {"role": "system", "content": get_test_case_reasoning_system_prompt()},
                {"role": "user", "content": get_test_case_reasoning_prompt(function_signature, test_input, expected_output)}
            ],
            max_tokens=2000,
            temperature=0.3
        )
