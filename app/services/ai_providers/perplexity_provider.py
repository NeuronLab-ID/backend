...
class _CookieExpiredError(Exception):
    """Raised when Perplexity returns 403, indicating expired cookies."""
...
class PerplexityProvider(AIProvider, SearchProvider):
    ...
    async def generate_hint(self, problem: dict, user_code: str, error: str) -> Optional[str]:
        ...
        prompt = get_perplexity_hint_prompt(problem_title, user_code[:500], error[:200])
    ...
    async def generate_reasoning(self, prompt: str, system_prompt: str = "") -> Optional[str]:
        ...
        augmentation = get_perplexity_reasoning_augmentation()
...
Include citations for your sources."""
    ...
    def _parse_thread_response(self, thread_data: dict) -> Optional[str]:
            ...
            if section_match:
                ...
                if block_type == "markdown":
                    ...
                    if isinstance(markdown_block, dict):
                        answer = markdown_block.get("answer", "")
                        if answer:
                            if section_num not in markdown_sections or len(answer) > len(
                                markdown_sections[section_num]
                            ):
                        ...
                        if media_items:
                            logger.debug(
                                f"[Perplexity] Section {section_num} markdown_block has {len(media_items)} media_items"
                            )
                        for item in media_items:
                            if item.get("medium") == "image":
                                ...
                                logger.info(
                                    f"[Perplexity] 📷 Section {section_num} markdown image: {img_name[:40]}... URL: {img_url[:80] if img_url else 'None'}..."
                                )
                                if img_url:
                    ...
                    if media_items:
                        logger.debug(
                            f"[Perplexity] Section {section_num} inline_entity_block has {len(media_items)} media_items"
                        )
                    for item in media_items:
                        if item.get("medium") == "image":
                            img_url = item.get("image") or item.get("url")
                            ...
                            logger.info(
                                f"[Perplexity] 📷 Section {section_num} inline image: {img_name[:40]}... URL: {img_url[:80] if img_url else 'None'}..."
                            )
                            if img_url:
                                if section_num not in section_images:
                                    section_images[section_num] = []
                    ...
                    if step.get("step_type") == "CODE":
                        ...
                        for asset in assets:
                            if asset.get("asset_type") == "CHART":
                                ...
                                logger.info(
                                    f"[Perplexity] 📷 CODE chart: URL: {chart_url[:80] if chart_url else 'None'}..."
                                )
        ...
        if markdown_sections:
            ...
            for section_num in sorted_sections:
                ...
                if section_num in section_images:
                    ...
                    logger.debug(
                        f"[Perplexity] Inserted {len(section_images[section_num])} image(s) after section {section_num}"
                    )
        ...
        if all_standalone:
            ...
            logger.info(
                f"[Perplexity] Appended {len(all_standalone)} visualization(s) ({len(standalone_images)} standalone, {len(orphan_images)} orphan)"
            )
    ...
    async def _request(
        self, query: str, model: str, sources: list, timeout: int, log_prefix: str, max_retries: int = 1
    ) -> Optional[str]:
        """Make request to Perplexity API with specified model and retry logic."""
        import uuid
        ...
        def _blocking_request():
            ...
            if resp.status_code == 403:
                ...
                raise _CookieExpiredError("cookies expired")
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    ...
                    logger.warning(
                        f"[{log_prefix}] Empty result, retrying in {delay + jitter:.1f}s (attempt {attempt + 1}/{max_retries})"
                    )
                ...
                if attempt < max_retries:
                    ...
                    jitter = delay * random.uniform(-0.2, 0.2)
                    logger.warning(
                        f"[{log_prefix}] Error: {error_msg}, retrying in {delay + jitter:.1f}s (attempt {attempt + 1}/{max_retries})"
                    )
                    await asyncio.sleep(delay + jitter)
                    continue

                logger.error(f"[{log_prefix}] Error after {max_retries + 1} attempts: {error_msg}")
                return None
